#!/bin/sh
# Runs the shared test suite on a platform where pixman is bundled with the
...
#
# Each run: packs src/Pixman.NET (skip with PIXMAN_SKIP_PACK=1 if artifacts/
...
#                 (or PIXMAN_BROWSER_PATH / PIXMAN_CHROMEDRIVER_DIR)
set -eu
...
device_tests=tests/apps/Pixman.NET.DeviceTests
wasm_tests=tests/apps/Pixman.NET.WasmTests
results="$root/artifacts/test-results/$platform"
config=${PIXMAN_TEST_CONFIGURATION:-Debug}
...
apple_platform_version() {
    if [ -n "${PIXMAN_APPLE_PLATFORM_VERSION:-}" ]; then
        echo "$PIXMAN_APPLE_PLATFORM_VERSION"
        return
    fi
...
if [ "${PIXMAN_SKIP_PACK:-}" != "1" ]; then
    rm -f artifacts/pixman-dotnet.*.nupkg artifacts/pixman-dotnet.*.snupkg
    dotnet pack src/Pixman.NET/Pixman.NET.csproj -c Release -o artifacts
fi
[ -f "artifacts/pixman-dotnet.$version.nupkg" ] || {
    echo "artifacts/pixman-dotnet.$version.nupkg is missing." >&2
    exit 1
}
...
case "$platform" in
    ios | tvos)
        ...
        dotnet build "$device_tests" -c "$config" -f "$tfm" -r "$rid" -p:PixmanApplePlatformVersion="$apple_version"
        ...
        xharness apple run --app "$app" --target "$target" --output-directory "$results" \
            --expected-exit-code 0 --timeout 00:15:00 --launch-timeout 00:05:00 ${PIXMAN_XHARNESS_ARGS:-}
        ;;
    ...
    maccatalyst)
        ...
        app=$(find "$device_tests/bin/$config/$tfm/$rid" -maxdepth 1 -name '*.app' | head -1)
        [ -n "$app" ] || { echo "no .app built under $device_tests/bin/$config/$tfm/$rid" >&2; exit 1; }
        xharness apple run --app "$app" --target maccatalyst --output-directory "$results" \
            --expected-exit-code 0 --timeout 00:15:00 ${PIXMAN_XHARNESS_ARGS:-}
        ;;

    android)
        tfm="net10.0-android"
        dotnet build "$device_tests" -c "$config" -f "$tfm" -p:PixmanApplePlatformVersion="${PIXMAN_APPLE_PLATFORM_VERSION:-26.5}"
        ...
        xharness android test --app "$apk" --package-name com.drasticactions.pixman.tests \
            --instrumentation pixman.tests.TestInstrumentation --output-directory "$results" \
            --expected-exit-code 0 --timeout 00:15:00 ${PIXMAN_XHARNESS_ARGS:-}
        ;;
    ...
    wasm)
        ...
        set --
        [ -n "${PIXMAN_BROWSER_PATH:-}" ] && set -- "$@" --browser-path "$PIXMAN_BROWSER_PATH"
        [ -n "${PIXMAN_CHROMEDRIVER_DIR:-}" ] && PATH="$PIXMAN_CHROMEDRIVER_DIR:$PATH"
        ...
        if ! command -v chromedriver >/dev/null 2>&1; then
            ...
            [ -n "${PIXMAN_BROWSER_PATH:-}" ] || set -- "$@" --browser-path "$chrome"
        fi
        # shellcheck disable=SC2086
        xharness wasm test-browser --app "$bundle" --browser Chrome "$@" \
            --output-directory "$results" --expected-exit-code 0 --timeout 00:15:00 ${PIXMAN_XHARNESS_ARGS:-}
        ;;
